...
export function MultiImageUploader({ values, slug, onChange, onPendingFilesChange }: { values: string[]; slug: string; onChange: (values: string[]) => void; onPendingFilesChange: (hasFiles: boolean, upload?: () => Promise<UploadedImage[]>) => void }) {
  ...
  const uploadPending = async (pending: LocalImagePreview[]) => {
    if (!pending.length) return [];
    pending.forEach((preview) => setPreviews((current) => current.map((item) => item.id === preview.id ? { ...item, status: "uploading" } : item)));
    const results = await Promise.all(pending.map((preview) => uploadImage(preview.file, { kind: "gallery", slug })));
    const nextValues = [...valuesRef.current, ...results.map((result) => result.secureUrl)];
    pending.forEach(revokeLocalImagePreview);
    previewsRef.current = [];
    valuesRef.current = nextValues;
    setPreviews([]);
    onChange(nextValues);
    onPendingFilesChange(false);
    onPendingFilesChange(false);
    return results;
  };
  ...
  const selectFiles = (files: FileList | File[]) => {
    ...
    setPreviews(next);
    onPendingFilesChange(true, () => uploadPending([...next]));
  };
  const removePreview = (id: string) => { cancelledIds.current.add(id); const removed = previewsRef.current.find((preview) => preview.id === id); const next = previewsRef.current.filter((preview) => preview.id !== id); if (removed) revokeLocalImagePreview(removed); previewsRef.current = next; setPreviews(next); onPendingFilesChange(next.length > 0, next.length ? () => uploadPending([...next]) : undefined); };
